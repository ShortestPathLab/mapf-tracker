...
const infoDescriptionText = {
  scenProgress: {
    ...
    x_axis:
      "Each map contains many different scenarios, and the x-axis shows the scenarios grouped by their type and ordered by the ID.",
    y_axis:
      "The y-axis shows the success rate (i.e., percentage) of closed, solved, and unknown instances in each scenario. " +
      "The success rate is calculated according to the total number of instances in each scenario.",
    comment:
      "To indicate the progress, all instances in each scenario are categorised into three types: " +
  ...
  agentProgress: {
    ...
    x_axis:
      "Each scenario contains instances with different numbers of agents. The x-axis shows the number of agents available in all scenarios in increasing order.",
    y_axis:
      "The y-axis shows the success rate (percentage) of closed, solved, and unknown instances for different numbers of agents. " +
  ...
  "scenCompare-#Instances Closed": {
    description:
      "This plot compares the number of instances closed by MAPF algorithms for each scenario within the given map. " +
    ...
    x_axis:
      "Each map contains many different scenarios, and the x-axis shows the scenarios grouped by their type and ordered by the ID.",
    y_axis:
      "The y-axis displays the number of instances closed for each scenario. " +
  ...
  "scenCompare-#Instances Solved": {
    description:
      "This plot compares the number of instances solved by MAPF algorithms for each scenario within the given map. " +
    ...
    x_axis:
      "Each map contains many different scenarios, and the x-axis shows the scenarios grouped by their type and ordered by the ID.",
    y_axis:
      "The y-axis displays the number of instances solved for each scenario. " +
      "The percentage ratio is shown, calculated based on the total number of instances in each scenario.",
  },
  "scenCompare-#Best Lower-bounds": {
    description:
      "This plot compares the number of instances that have achieved the best lower bound (reported by any algorithm) among MAPF algorithms for each scenario in the given map. " +
    ...
    x_axis:
      "Each map contains many different scenarios, and the x-axis shows the scenarios grouped by their type and ordered by the ID.",
    y_axis:
      "The y-axis displays the number of instances that have achieved the best lower bound for each scenario. " +
      "The percentage ratio is shown, calculated based on the total number of instances in each scenario. ",
  ...
  "scenCompare-#Best Solutions": {
    description:
      "This plot compares the number of instances that have achieved the best solution (reported by any algorithm) among MAPF algorithms for each scenario in the given map. " +
    ...
    x_axis:
      "Each map contains many different scenarios, and the x-axis shows the scenarios grouped by their type and ordered by the ID.",
    y_axis:
      "The y-axis displays the number of instances that have achieved the best solution for each scenario. " +
      "The percentage ratio is shown, calculated based on the total number of instances in each scenario. ",
  ...
  "agentCompare-#Instances Closed": {
    ...
    x_axis:
      "Each scenario contains instances with different numbers of agents. The x-axis shows the number of agents available in all scenarios in increasing order.",
    y_axis:
      "The y-axis displays the number of instances closed for different numbers of agents. " +
      "For each different number of agents, we first count the number of instances that have the same number of agents in all scenarios. " +
  ...
  "agentCompare-#Instances Solved": {
    ...
    x_axis:
      "Each scenario contains instances with different numbers of agents. The x-axis shows the number of agents available in all scenarios in increasing order.",
    y_axis:
      "The y-axis displays the number of instances solved for different numbers of agents. " +
      "For each different number of agents, we first count the number of instances that have the same number of agents in all scenarios. " +
      "The percentage ratio is shown, calculated based on the number of instances available for each different number of agents.",
  },
  "agentCompare-#Best Lower-bounds": {
    ...
    x_axis:
      "Each scenario contains instances with different numbers of agents. The x-axis shows the number of agents available in all scenarios in increasing order.",
    y_axis:
      "The y-axis displays the number of instances that have achieved the best lower bound for different numbers of agents. " +
      "For each different number of agents, we first count the number of instances that have the same number of agents in all scenarios. " +
      "The percentage ratio is shown, calculated based on the number of instances available for each different number of agents. ",
    // "For instances where no lower bound is reported, no algorithm can achieve the best lower bound in such cases."
  },
  "agentCompare-#Best Solutions": {
    ...
    x_axis:
      "Each scenario contains instances with different numbers of agents. The x-axis shows the number of agents available in all scenarios in increasing order.",
    y_axis:
      "The y-axis displays the number of instances that have achieved the best solution for different numbers of agents. " +
      "For each different number of agents, we first count the number of instances that have the same number of agents in all scenarios. " +
      "The percentage ratio is shown, calculated based on the number of instances available for each different number of agents. ",
  ...
  fetch(APIConfig.apiUrl + "/scenario/map/" + id, { method: "GET" })
    ...
    .then((data) => {
      data.forEach(function (element) {
        element.solved_percentage =
          element.instances_solved / element.instances;
        element.closed_percentage =
          element.instances_closed / element.instances;
      });
      callback(data);
    })
...
export default function ScenarioTable() {
  ...
  const [mapBarChartDisplayAlgorithms, setMapBarChartDisplayAlgorithms] =
    React.useState([]);
  ...
  const [progressColor] = React.useState({
    Unknown: "#BF0A30",
    Solved: "#F9812A",
    Closed: "#4CBB17",
  });
  ...
  const navigateToInstance = (event, id, scen_id, scen_type, num_agents) => {
    ...
    navigate("/instances", {
      state: {
        ...
        scenTypeID: scen_id,
        scenType: scen_type,
        mapName: location.state.mapName,
  ...
  const navigateToDownload_Benchmark = (
    event,
    object_id,
    scen_type,
    type_id
            ...
            {payload.map((entry, index) => (
              <li key={`item-${index}`} style={{ color: entry.color }}>
                {`${entry.name}: ${Math.round(
                  entry.value * total
                )} (${getPercent(Math.round(entry.value * total), total)})`}
              </li>
            ))}
          </ul>
        </div>
    ...
    if (mapQueryResult.length > 0) {
      ...
      [
        ...new Set(
          data.map(
            (item) =>
              (item.scen_type === "random" ? "rand" : "even") +
              " " +
              item.type_id
          )
        ),
      ].forEach((element) => mapChartData.push({ name: element }));
      ...
      for (var i = 0; i < mapQueryResult.length; i++) {
        ...
        var mapIndex = mapChartData.findIndex(
          (x) =>
            x.name ===
            (mapQueryResult[i].scen_type === "random" ? "rand" : "even") +
              " " +
              mapQueryResult[i].type_id
        );
    ...
    if (agentQueryResult.length > 0) {
      ...
      for (var i = 0; i < agentQueryResult.length; i++) {
        ...
        for (var j = 0; j < agentQueryResult[i].solved_instances.length; j++) {
          ...
          algorithm.add(algo.algo_name);
          agent_data[agentIndex][algo.algo_name] =
  ...
  const handleClickOpenScenProgress = (event, scrollType) => {
    ...
    data.forEach((element) =>
      progressChartData.push({
        name:
          (element.scen_type === "random" ? "rand" : "even") +
          " " +
          element.type_id,
  ...
  const handleClickOpenAgentProgress = (event, scrollType) => {
    ...
    setScroll(scrollType);
    fetch(APIConfig.apiUrl + "/instance/test/" + location.state.mapId, {
      method: "GET",
    })
      .then((res) => res.json())
      .then((data) => {
          ...
          >
            <MenuList>
              <MenuItem
                key="M_scen"
                onClick={(event) => {
                  handleClickOpenScenProgress(event, "paper");
                  setMenuAnchorEl(null);
                  setSubAnchorEl(null);
                }}
              >
              ...
              <MenuItem
                key="M_agents"
                onClick={(event) => {
                  handleClickOpenAgentProgress(event, "paper");
                  setMenuAnchorEl(null);
                  setSubAnchorEl(null);
                }}
              >
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell align="center">
                        ...
                        >
                          {loadingBenchmark && row.id === query_id ? (
          ...
          >
            ...
            >
              <Typography
                ...
                component="div"
              >
                Success Rate on #Agents (
                {capitalizeFirstLetter(location.state.mapName)})
                <IconButton
                  onClick={() => {
                    handleOpenInfo("agentProgress");
                  }}
                >
                  <InfoIcon />
                </IconButton>
                    ...
                    onClick={(event) => {
                      setMapFilterState({
                        ...
                        [event.currentTarget.innerText]:
                          !mapFilterState[event.currentTarget.innerText],
                      });
                    }}
                  >
                    <Checkbox
            ...
            {mapLoading ? (
              ...
              <ResponsiveContainer width="100%" height={400}>
                ...
                >
                  ...
                  <Tooltip
                    content={renderScenTooltipContent}
                    wrapperStyle={{
                      backgroundColor: "white",
                      borderStyle: "ridge",
                      paddingLeft: "10px",
