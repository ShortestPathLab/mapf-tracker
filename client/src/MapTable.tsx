...
export default function MapTable({ showHeader = true }) {
  ...
  const [domainBarChartAlgorithms, setDomainBarChartAlgorithms] =
    React.useState([]);
  const [domainBarChartDisplayAlgorithms, setDomainBarChartDisplayAlgorithms] =
    React.useState([]);
  const [domainBarChartDisplayData, setDomainBarChartDisplayData] =
    React.useState([]);
  ...
  React.useEffect(() => {
    if (loading && query_id !== "") {
      fetch(APIConfig.apiUrl + "/instance/DownloadMapByID/" + query_id, {
        method: "GET",
      })
        .then((res) => res.json())
        .then((data) => {
  ...
  const navigateToInstances = (event, id, map_name) => {
    ...
    navigate("/scenarios", {
      state: { mapId: id, mapName: map_name },
      replace: false,
    });
    event.stopPropagation();
        ...
        >
          ...
          <ul className="list">
            <li key={`item-0`} style={{ color: "#4CBB17" }}>
              {`Closed: ${Math.round(closed * total)} (${getPercent(
                Math.round(closed * total),
                total
              )})`}
            </li>
            <li key={`item-1`} style={{ color: "#F9812A" }}>
              {`Solved: ${Math.round((solved - closed) * total)} (${getPercent(
                Math.round((solved - closed) * total),
                total
              )})`}
            </li>
            <li key={`item-2`} style={{ color: "#BF0A30" }}>
              {`Unknown: ${Math.round((1 - solved) * total)} (${getPercent(
                Math.round((1 - solved) * total),
                total
              )})`}
            </li>
          </ul>
  ...
  React.useEffect(() => {
    if (algorithm_name.length > 0) {
      ...
      Promise.all([
        ...
        .then(([closed_data, domain_chart_data]) => {
  ...
  const handleClickOpenDomainProgress = (event, scrollType) => {
    ...
    data.forEach(function (element) {
      if (!(element.map_type in domainChartData)) {
        domainChartData[element.map_type] = {
          name:
            element.map_type.charAt(0).toUpperCase() +
            element.map_type.slice(1),
        ...
        domainChartData[element.map_type].total =
          domainChartData[element.map_type].total + element.instances;
        domainChartData[element.map_type].Closed =
          domainChartData[element.map_type].Closed + element.instances_closed;
        domainChartData[element.map_type].Solved =
          domainChartData[element.map_type].Solved +
          element.instances_solved -
          element.instances_closed;
        domainChartData[element.map_type].Unknown =
          domainChartData[element.map_type].Unknown +
          element.instances -
          element.instances_solved;
      }
    ...
    for (const key in domainChartData) {
      domainChartData[key].Closed =
        domainChartData[key].Closed / domainChartData[key].total;
      domainChartData[key].Solved =
        domainChartData[key].Solved / domainChartData[key].total +
        domainChartData[key].Closed;
  ...
  return (
    <>
      <Stack sx={{ mx: "auto", width: 1488, gap: 4, py: 6 }}>
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell align="center">
                          <IconButton
                            onClick={(event) =>
                              handleMapDetailClickOpen(event, "paper", row)
                            }
                          >
                            <InfoIcon />
                          </IconButton>
                        ...
                        <TableCell align="center">
                          ...
                          >
                            {benchmarkLoading && row.id === query_id ? (
                      ...
                      onClick={(event) => {
                        setDomainFilterState({
                          ...
                          [event.currentTarget.innerText]:
                            !domainFilterState[event.currentTarget.innerText],
                        });
                      }}
                    >
                      <Checkbox
                ...
                <ResponsiveContainer width="100%" height={445}>
                  ...
                  >
                    ...
                    <PolarRadiusAxis
                      ...
                      domain={[
                        0,
                        domainBarChartDisplayAlgorithms.length > 0
                          ? "dataMax"
                          : 1,
                      ]}
                      stroke={"black"}
            ...
            <Box sx={{ width: "100%" }}>
              ...
              >
                ...
                >
                  ...
                  <IconButton
                    onClick={() => {
                      handleOpenInfo("mapCompare-" + mapQuery);
                    }}
                  >
                    <InfoIcon />
                  </IconButton>
        ...
        >
          <DialogContent
            dividers={scrollMapDetail === "paper"}
