...
export default function UserMapPage() {
  ...
  const [submissionFeedbackText, setSubmissionFeedbackText] =
    React.useState("");
  ...
  const validate_path = function (solution, map) {
    ...
    for (var i = 0; i < solution.length; i++) {
      for (var t = 1; t < solution[i]; t++) {
        ...
        if (
          Math.abs(solution[i][t].x - solution[i][t - 1].x) +
            Math.abs(solution[i][t].y - solution[i][t - 1].y) >
          1
        ) {
          // invalid move;
          return false;
      ...
      for (var j = 0; j < solution.length; j++) {
        ...
        for (; t < a1.length; t++) {
          if (a1[t] === a2[t]) {
            // vertex conflict
            return false;
          } else if (a1[t] === a2[t - 1] && a1[t - 1] === a2[t]) {
            // edge conflict
            return false;
          }
        }
    ...
    return new Promise(async function (complete, error) {
      ...
      Papa.parse(files[0], {
          ...
          if (counter === 0) {
            ...
            if (results.data.length !== 7) {
              ...
              current_display_string = current_display_string.concat(
                "Row: ",
                counter,
                " Expected 7 entries, some entries are missing\n"
              );
              setValidationText(current_display_string);
              parser.abort();
            } else {
              ...
              if (results.data[1] !== "even" && results.data[1] !== "random") {
                ...
                current_display_string = current_display_string.concat(
                  "Row: ",
                  counter,
                  " Type must equal to even or random\n"
                );
                setValidationText(current_display_string);
                parser.abort();
              }
              ...
              if (results.data[3] === "") {
                ...
                current_display_string = current_display_string.concat(
                  "Row: ",
                  counter,
                  " Number of agents must not be empty\n"
                );
                setValidationText(current_display_string);
                parser.abort();
              } else {
                if (parseInt(results.data[3]) <= 0) {
                  ...
                  current_display_string = current_display_string.concat(
                    "Row: ",
                    counter,
                    " Number of agents must larger than 0\n"
                  );
                  setValidationText(current_display_string);
                  parser.abort();
                }
                if (parseInt(results.data[3]) > selectedRow.instances) {
                  ...
                  current_display_string = current_display_string.concat(
                    "Row: ",
                    counter,
                    " Number of agents must less than ",
                    selectedRow.instances,
                    "\n"
                  );
                  setValidationText(current_display_string);
                  parser.abort();
              ...
              if (results.data[4] !== "" && results.data[5] !== "") {
                if (parseInt(results.data[4]) > parseInt(results.data[5])) {
                  ...
                  current_display_string = current_display_string.concat(
                    "Row: ",
                    counter,
                    " Lower bound must less than soution cost\n"
                  );
                  setValidationText(current_display_string);
                  parser.abort();
                }
              ...
              if (results.data[5] !== "" && results.data[6] !== "") {
                if (
                  results.data[6].length - parseInt(results.data[3]) + 1 !==
                  parseInt(results.data[5])
                ) {
                  foundError = true;
                  current_display_string = current_display_string.concat(
                    "Row: ",
                    counter,
                    " Solution cost not match with the solution plan\n"
                  );
                  setValidationText(current_display_string);
                  parser.abort();
                }
        ...
        complete: function () {
          if (foundError === false) {
            current_display_string = current_display_string.concat(
              "Finished validation, total number of row: ",
              counter - 1
            );
            setValidationText(current_display_string);
          } else {
            current_display_string = current_display_string.concat(
              "Validation fail, please resubmit your csv"
            );
            setValidationText(current_display_string);
          }
          return complete({ pass: !foundError, row_number: counter - 1 });
      ...
      Papa.parse(files[0], {
        ...
        complete: function () {
          ...
          if (foundError === false) {
            ...
            setSubmissionFeedbackText(
              "Submission failed, please resubmit your csv"
            );
          }
    ...
    <Box sx={{ width: "96%", paddingLeft: "2%", opacity: "0.95" }}>
          ...
          <Table>
            ...
            <TableBody>
              ...
              <TableRow>
                ...
                <TableCell>38</TableCell>
                <TableCell>urrurrruuurrrr ddrddrrrddrddrdrrdrddddd</TableCell>
              </TableRow>
              <TableRow>
                <TableCell>empty-32-32</TableCell>
                <TableCell>even</TableCell>
                ...
                <TableCell>50</TableCell>
                <TableCell>
                  urrurrruuurrrr ddrddrrrddrddrdrrdrddddd dddddddddddd
                </TableCell>
              </TableRow>
            </TableBody>
          </Table>
      ...
      >
        ...
        <DialogContent dividers>
          ...
          {showProgress ? (
            <BorderLinearProgress value={submissionProgress * 100} />
          ) : null}
