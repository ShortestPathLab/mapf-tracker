...
See <Link href="/docs/solution-format">here</Link> for the accepted solution format.
...
Here's an example submitting a CSV with cURL.

<CodeBlock language="bash">
{`
    curl -X POST <<ENDPOINT HERE>> \\
       -H "Content-Type: text/csv" \\
       --data-binary @solution.csv
`.trim()}
</CodeBlock>

###### Submitting with Python

Here's an example submitting a YAML file with Python.

<CodeBlock language="python">
{`
    with open("solution.yaml, "r") as file:
