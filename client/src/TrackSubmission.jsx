  ...
  return (
    <g transform={`translate(${x + (x - cx) / 16},${y + (y - cy) / 16})`}>
      <text
        ...
        textAnchor={"middle"}
        transform={`rotate(${angle[payload.value] === undefined ? 0 : angle[payload.value]
          })`}
      >
        {payload.value}
      </text>
...
export default function TrackSubmission() {
  ...
  const handleRequestDetailUpdated = async (values, { setSubmitting, resetForm }) => {
    try {
            ...
            <Form>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, justifyContent: 'flex-end' }}>
                <Field
                  ...
                  sx={{ width: '250px' }}
                />
                <Button type="submit" variant="contained" color="success" sx={{ width: '100px', height: '35px' }} disabled={isSubmitting}>
                  Show
                </Button>
              </Box>
            </Form>
                ...
                .map((row, index) => {
                  ...
                  return (
                    <TableRow
                      ...
                      tabIndex={-1}
                      key={row.id}
                      onClick={(event) => { navigateToSubmissionSummary(event, row) }}
                    >
                      <TableCell
                        id={labelId}
                        scope="row"
            ...
            >
              {({ isSubmitting }) => (
                <Form style={{ width: '100%', marginTop: 10 }}>
                  ...
                  <Box sx={{ display: 'flex', gap: 5, marginBottom: 5, justifyContent: 'center' }}>
