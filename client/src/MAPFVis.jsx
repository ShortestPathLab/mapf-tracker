...
import ShutterSpeedIcon from "@mui/icons-material/ShutterSpeedOutlined";
...
const Visualization = () => {
  ...
  const [currCanvasConfig, setCurrCanvasConfig] = useState({
    width: 750,
    height: 750,
  });
  ...
  useEffect(() => {
    fetch(`${APIConfig.apiUrl}/solution_path/${location.state.path_id}`, {
      method: "GET",
    })
      .then((res) => res.json())
      .then((data) => {
  ...
  useEffect(() => {
    if (imageData === null && map.length > 1) {
      ...
      var image = ctx.getImageData(
        0,
        0,
        currCanvasConfig.width,
        currCanvasConfig.height
      );
      setImageData(image);
  ...
  const handleChangeCanvas = (value) => () => {
    ...
    if (current_scale > 0) {
      ...
      setCurrCanvasConfig({
        width: canvasConfig.width * current_scale,
        height: canvasConfig.height * current_scale,
      });
    }
  };
    ...
    <div className="container">
      ...
      <div style={{ width: "100%", height: "100%" }}>
        <div style={{ display: "grid", margin: "0 auto" }}>
          <canvas
            ref={invisiblecanvasRef}
            width={0}
            height={0}
            style={{ zIndex: 0, gridArea: "2 / 2 / 2 / 2" }}
          />
          <canvas
            ref={canvasRef}
            width={currCanvasConfig.width}
            height={currCanvasConfig.height}
            style={{
          ...
          <canvas
            ...
            width={currCanvasConfig.width}
            height={currCanvasConfig.height}
            style={{ zIndex: 2, gridArea: "2 / 2 / 2 / 2" }}
          />
        </div>
